...
export default function DispatchSummaryList() {
  const { canAccess } = useRBAC();
  const canEdit = canAccess('Admin', 'Owner', 'Office_POC', 'Warehouse_POC', 'PO_Executive');

  const [vendorTab, setVendorTab] = useState('');
  const [filters, setFilters] = useState(defaultFilters);
  const [sort, setSort] = useState({ key: 'updated_at', dir: 'desc' });
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(() => loadPersistedPageSize('dispatchSummary', 25));
  ...
  const load = useCallback((overrides) => {
    setLoading(true);
    listDispatchSummary(buildParams(overrides))
      .then(res => { setItems(res.rows || []); setTotal(res.total || 0); })
      .catch(() => toast.error('Failed to load dispatch summary'))
  ...
  const handlePageSizeChange = (size) => {
    setPageSize(size);
    persistPageSize('dispatchSummary', size);
    setPage(1);
  ...
  const saveRow = async (po, { confirmDuplicate = false } = {}) => {
    ...
    try {
      const payload = {};
      if ('dispatch_date' in e) payload.dispatch_date = e.dispatch_date || null;
      ...
      await updateDispatchRow(po.po_id, payload);
  ...
  const bulkHasChanges = bulkCourierId !== '' || !!bulkDispatchDate;
  const bulkApplyDisabled = selected.size === 0 || bulkApplying || !bulkHasChanges;
  const applyBulk = async () => {
    if (bulkApplyDisabled) return;
    setBulkApplying(true);
    try {
      const payload = { po_ids: Array.from(selected) };
      if (bulkCourierId !== '')     payload.courier_id    = bulkCourierId === 'clear' ? null : Number(bulkCourierId);
      if (bulkDispatchDate)         payload.dispatch_date = bulkDispatchDate;
      await bulkUpdateDispatch(payload);
  ...
  const downloadXLSX = async () => {
    ...
    try {
      ...
      const res = await listDispatchSummary(params);
      const rows = res.rows || [];
      if (rows.length === 0) {
      ...
      const data = rows.map((r, i) => exportCols.map(c => {
        if (c.key === '__sno') return i + 1;
        const v = r[c.key];
        return v == null ? '' : v;
      }));
      ...
      XLSX.writeFile(wb, `dispatch-summary-${vendorTab || 'all'}-${stamp}.xlsx`);
    } catch (err) {
  ...
  return (
    <AppShell>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#003049]">Dispatch Summary</h1>
      ...
      {canEdit && selected.size > 0 && (
        <div className="bg-[#003049] text-white rounded-xl px-4 py-3 mb-3 flex items-center gap-3 flex-wrap sticky top-2 z-20 shadow">
          ...
          <label className="text-sm whitespace-nowrap">Set Courier</label>
          <select
            value={bulkCourierId}
          ...
          <label className="text-sm whitespace-nowrap">Dispatch Date</label>
          <input
            type="date"
          ...
          <Button
            ...
            title={!bulkHasChanges ? 'Set a courier or dispatch date to apply' : undefined}
          >
            Apply
          </Button>
          <button
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 bg-gray-50">
              <tr className="border-b border-gray-200">
                ...
                <th className="px-3 py-3 text-left font-semibold text-gray-600 whitespace-nowrap bg-gray-50 w-16">S.No.</th>
                {COLUMNS.map(col => (
            ...
            <tbody>
              {loading ? (
                [...Array(4)].map((_, i) => (
                  <tr key={i}>
                    <td colSpan={COLUMNS.length + 1 + (canEdit ? 2 : 0)} className="px-4 py-3">
                ...
                return (
                  <tr key={po.po_id} className={`border-b border-gray-100 ${dirty ? 'bg-amber-50/60' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-3 py-2 text-gray-500 font-mono text-xs">{sno}</td>
                    {COLUMNS.map(col => {
                      switch (col.key) {
                        case 'po_date':
                        case 'vendor_po_id':
                            ...
                            <td key={col.key} className="px-3 py-2">
                              {canEdit ? (
                                ...
                                >
                                  ...
                                  {activeCouriers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                  {/* Preserve a previously-assigned inactive courier as a visible option */}
