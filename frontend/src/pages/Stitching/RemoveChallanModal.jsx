...
/**
 * Withdraw a challan that should not exist.
 *
...
export default function RemoveChallanModal({ challan, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success('Challan withdrawn');
      onSaved?.();
      onClose();
    } catch (err2) {
      toast.error(err2.response?.data?.message || 'Could not withdraw the challan');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal isOpen onClose={onClose} title="Withdraw challan" size="md">
      <form onSubmit={submit}>
        <div className="rounded-lg bg-gray-50 border border-gray-200 px-4 py-3 mb-4 text-sm text-gray-600">
          ...
          to {challan.party_name} stops counting as dispatched, and that quantity is available again
          on the lot it was taken from. The challan is kept as a record, struck through, not deleted.
        </div>
        ...
        <textarea
          ...
          placeholder="e.g. entered against the wrong PO"
        />

        <p className="mt-2 text-[11px] text-gray-400">
          Use this when the challan should not exist — recorded against the wrong lot, or never
          actually raised. Then enter it against the right one.
        </p>
