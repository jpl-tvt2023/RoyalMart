...
async function create(req, res, next) {
  try {
    ...
    const { rows } = await db.execute({
      sql: 'INSERT INTO categories (name) VALUES (?) RETURNING id, name, is_active, created_at',
      args: [name],
    });
    await logAction({
      userId: req.user.id,
...
async function update(req, res, next) {
  try {
    ...
    const { rows: existing } = await db.execute({
      sql: 'SELECT id, name, is_active FROM categories WHERE id = ?',
      args: [id],
    });
    ...
    const { rows } = await db.execute({
      sql: `UPDATE categories SET name = ?, is_active = ?, updated_by = ?, updated_at = datetime('now')
    ...
    await logAction({
      ...
      description: `Updated category #${id}: name="${nextName}", active=${nextActive}`,
      entityType: 'category',
      entityId: id,
      changes,
    });
...
async function remove(req, res, next) {
  try {
    ...
    if (!existing.length) return res.status(404).json({ message: 'Category not found' });
    const { rows } = await db.execute({
      sql: 'UPDATE categories SET is_active = 0 WHERE id = ? RETURNING id, name, is_active, created_at',
      args: [id],
    });
    await logAction({
      userId: req.user.id,
      actionType: 'CATEGORY_DEACTIVATE',
      description: `Deactivated category "${existing[0].name}"`,
      entityType: 'category',
      entityId: id,
    });
    res.json(rows[0]);
